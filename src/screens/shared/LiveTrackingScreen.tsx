...
const LiveTrackingScreen: React.FC = () => {
  ...
  const { refreshRequests } = useBloodRequest();
  ...
  /**
   * Load request data with redirect prevention
   */
  const loadRequestData = async () => {
    // Prevent loading if already redirecting
    if (isRedirecting) {
      console.log('⏸️ [LiveTracking] Already redirecting, skipping load');
      return;
    }
    ...
    try {
      ...
      // If request is cancelled or completed, redirect IMMEDIATELY (no loops)
      if (requestData.status === 'CANCELLED' || requestData.status === 'COMPLETED') {
        console.log(`⚠️ [LiveTracking] Request ${requestData.status}, redirecting IMMEDIATELY`);
        ...
        if (requestData.status === 'COMPLETED') {
          // Check if user has already rated or skipped before showing rating screen
          try {
            const ratingStatus = await respectAPI.checkRatingStatus(
              requestId,
              isDonor ? user!.id : donorId!,
              requestData.recipientId,
              isDonor ? 'donor' : 'recipient'
            );

            console.log('🔍 [LiveTracking] Rating status:', ratingStatus);

            if (ratingStatus.shouldShowRating) {
              // Navigate to rating screen for completed requests
              navigation.replace('RatingScreen' as never, {
                requestId,
                donorId: isDonor ? user!.id : donorId,
                recipientId: requestData.recipientId,
                raterRole: isDonor ? 'donor' : 'recipient',
              } as never);
            } else {
              // Already rated or skipped, go directly to history
              console.log('⏭️ [LiveTracking] User already rated/skipped, going to history');
              navigation.replace(isDonor ? 'RequestHistory' : 'RequestHistory' as never);
            }
          } catch (error) {
            console.error('❌ [LiveTracking] Error checking rating status:', error);
            // On error, default to showing rating screen
            navigation.replace('RatingScreen' as never, {
              requestId,
              donorId: isDonor ? user!.id : donorId,
              recipientId: requestData.recipientId,
          ...
          // Show notification about who cancelled (IMMEDIATE redirect)
          const cancelledByMe = requestData.cancelledBy === user!.id;
          const cancellerRole = requestData.cancelledByRole;
          
          if (!cancelledByMe) {
            // Show alert that the other party cancelled
            const otherParty = cancellerRole === 'donor' ? 'donor' : 'recipient';
            showAlert({
              title: 'Request Cancelled',
              message: `This request has been cancelled by the ${otherParty}.${requestData.cancellationReason ? `\n\nReason: ${requestData.cancellationReason}` : ''}`,
              type: 'warning'
            });
          }
          
          // Navigate IMMEDIATELY to home for cancelled requests
          navigation.replace(isDonor ? 'DonorHome' : 'UserHome' as never);
        }
        return;
      }
      ...
      const API_BASE_URL = 'http://https://bdms-production-5878.up.railway.app/api';
  ...
  /**
   * Reload on screen focus - ONLY if not redirecting
   */
  useFocusEffect(
    React.useCallback(() => {
      if (!isRedirecting) {
  ...
  /**
   * Poll for updates - FAST (2 seconds) - STOPS when redirecting
   */
  useEffect(() => {
    const interval = setInterval(() => {
      if (!isPollingPaused && !isSubmitting && !isRedirecting) {
        loadRequestData();
      }
    }, 2000); // Every 2 seconds for real-time experience

    return () => clearInterval(interval);
  }, [requestId, isPollingPaused, isSubmitting, isRedirecting]);
  ...
  const handleToggleLocationSharing = async () => {
    ...
    try {
      const response = await fetch(`http://https://bdms-production-5878.up.railway.app/api/blood-requests/${requestId}/share-location`, {
        method: 'PATCH',
  ...
  /**
   * Handle Complete - IMMEDIATE REDIRECT (NO LOOPS)
   */
  const handleComplete = async () => {
    ...
    try {
      ...
      console.log('✅ [LiveTracking] Complete API response:', result);
      ...
      console.log('✅ [LiveTracking] Blood request context refreshed');
      ...
      try {
        ...
        console.log('🔍 [LiveTracking] Rating status after complete:', ratingStatus);
        ...
        if (ratingStatus.shouldShowRating) {
          ...
          // Already rated or skipped, go to history
          console.log('⏭️ [LiveTracking] User already rated/skipped, going to history');
          navigation.replace(isDonor ? 'RequestHistory' : 'RequestHistory' as never);
        }
      } catch (error) {
        console.error('❌ [LiveTracking] Error checking rating status:', error);
        // On error, default to going to history
        navigation.replace(isDonor ? 'RequestHistory' : 'RequestHistory' as never);
      }
    } catch (error: any) {
      ...
      setIsRedirecting(false); // Allow polling again
    } finally {
  ...
  /**
   * Handle Cancel - IMMEDIATE REDIRECT (NO LOOPS)
   */
  const handleCancel = async () => {
    ...
    try {
      ...
      console.log('❌ [LiveTracking] Cancel API response:', result);
            ...
            <View style={styles.infoRow}>
              <View style={styles.infoItemFull}>
                ...
                <Text style={styles.infoValueLarge} numberOfLines={3}>
                  {(() => {
                    // Check if location looks like coordinates (numbers, dots, commas only)
                    if (!request.location || request.location.match(/^[-\d.,\s]+$/)) {
                      // Show formatted coordinates if available
                      if (request.recipientLatitude && request.recipientLongitude) {
                        return `Location on map\n(${request.recipientLatitude.toFixed(4)}, ${request.recipientLongitude.toFixed(4)})`;
                      }
                      return 'See map below';
                    }
